...
describe('vehiculos', () => {
    ...
    test('of getVehiculos ByKmRange and all Make are returned as json', async () => {
        ...
        await api
            .get(`/api/vehiculos/?from=${fromFiltro}&to=${toFiltro}`)
            .expect(200)
            .expect('Content-Type', /application\/json/);
    ...
    test('of getVehiculos ByKmRange [105k to 110k] and all Make sortenning', async () => {
        const onlyTextFilter = await api.get('/api/vehiculos/?from=105000&to=110000');
    ...
    test('of getVehiculos ByKmRange [] and a Make [Toyota] sortenning', async () => {
        const fullFilter = await api.get('/api/vehiculos/?marca=Hyundai');
        expect(fullFilter.body).toHaveLength(2);
        expect(fullFilter.body[0].propietario).toBe('Sayre MacSwayde');
        expect(fullFilter.body[1].propietario).toBe('Kimbell Dalzell');
    });
});
